...
public class Darstellungscanvas extends Canvas implements Beobachter {
  ...
  /**
   * Ein internes Mapping zwischen Orten und Objekten, das verwendet wird,
   * damit die Objekte am gleichen Ort
   * untereinander (und nicht übereinander) dargestellt werden.
   */
  private Map<ISimObjekt.Ort, Integer> cacheSimObjekteInOrt;

  public Darstellungscanvas(
    int breite, int hoehe, List<ISimObjekt> simObjekte
  ) {
    super(breite, hoehe);
    ...
    try {
      FileInputStream inputstream = new FileInputStream(
        "src/main/resources/sprites/schiff.png");
  ...
  /**
   * Ermittelt die passende y-Position für ein Objekt, sodass die Objekte
   ...
   */
  private void zeichneZustandFortschritt(
    GraphicsContext gc, double x, int y, ISimObjekt simObjekt
  ) {
  ...
  /**
   * Ermittelt den Ort an dem ein Objekt gezeichnet wird (Pirat bei dem
   * Schiff, wenn dessen Ort AUF-SCHIFF ist).
   */
    ...
    switch (ort) {
      ...
      default:
        throw new IllegalArgumentException(
          "Dieser Ort sollte nicht " + "als Zeichenort verwendet werden.");
    }
  }
  ...
  public void update(Object payload) {
    Platform.runLater(this::allesZeichnen);
  }
}
